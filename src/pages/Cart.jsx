...
import { ClearCart, removeCartItem, plus, minus } from '../redux/actions/cart'
...
function Cart() {
    ...
    const groupPizzas = Object.keys(items).map(pizza => {
        return items[pizza].items[0];
    });
    const onClearCart = () => {
        if (window.confirm('Реали хотите дропнуть корзину?')) {
            dispatch(ClearCart());
        }
    }
    const removeItem = (id) => {
    ...
    return (
     ...
     Корзина</h2>
                    ...
                    <div className="content__items">
                        {
                            groupPizzas.map((pizza, index) => <CartItem key={`${index}_${pizza.name}`} {...pizza} totalPrice={items[pizza.id].totalPrice} totalCount={items[pizza.id].items.length} remove={removeItem} plus={onPlus} minus={onMinus} />)
                        }
                    </div>
